...
impl RecorderManager {
    ...
    pub async fn start_recording_with_emitter(
        ...
        tokio::spawn(async move {
            ...
            manager
                .finish_file(username, session_dir, emitter, record_duration_secs)
                .await;
        });
    ...
    /// Merge a completed file without interrupting the ongoing download session.
    async fn finish_file(
        self: &Arc<Self>,
        username: String,
        session_dir: PathBuf,
        emitter: Arc<dyn Emitter>,
        record_duration_secs: Option<u64>,
    ) {
        let manager = self;
        ...
        emitter.emit(
            ...
            &serde_json::json!({
                ...
                "is_recording": manager.is_recording(&username),
            }),
        );
    }
    ...
    fn rotate_file(
        ...
        let next_dir = loop {
            ...
            if !candidate.exists()
                ...
                && !parent.join(format!("{}.mkv", stem)).exists()
        ...
        let target = parent.join(format!(
            "{}.{}",
            next_dir.file_name().unwrap().to_string_lossy(),
            format
        ));
        crate::recording::meta::ensure_meta(&target, &now.to_rfc3339());
        crate::recording::meta::set_status(&target, "recording");
        let previous = std::mem::replace(session_dir, next_dir);
        // Lock the completed directory before releasing its active-session protection.
        self.waiting_merge_dirs.write().insert(previous.clone());
        let duration = self.sessions.write().get_mut(username).map(|session| {
            let elapsed = now
                .signed_duration_since(session.started_at)
                .num_seconds()
                .max(0) as u64;
            session.dir_path = session_dir.clone();
        ...
        tokio::spawn(async move {
            manager
                .finish_file(username, previous, emitter, duration)
                .await;
        });
        Ok(())
    }
...
/// Zero disables rotation. Duration is measured from successfully recorded media.
...
mod duration_tests {
    use super::*;
    use crate::core::emitter::BroadcastEmitter;

    #[test]
    fn unlimited_and_live_limit_changes() {
        assert!(!recording_limit_reached(7200.0, 0));
        assert!(!recording_limit_reached(1799.5, 1800));
        assert!(recording_limit_reached(1800.0, 1800));
        assert!(recording_limit_reached(1802.5, 1800));
        // A lower saved limit applies to media already recorded in this file.
        assert!(recording_limit_reached(900.0, 600));
        assert!(!recording_limit_reached(900.0, 1800));
        assert!(!recording_limit_reached(0.0, 1));
    }

    #[tokio::test]
    async fn rotation_preserves_session_and_stop_channel() {
        let root =
            std::env::temp_dir().join(format!("recorder-rotation-{}", rand::random::<u64>()));
        let mut dir = root.join("test_20260101_000000");
        fs::create_dir_all(&dir).unwrap();
        let state = AppState::new().unwrap();
        state.data.write().settings.output_dir = root.to_string_lossy().into_owned();
        state.data.write().pipeline = Default::default();
        let manager = RecorderManager::new(state);
        ...
        let emitter: Arc<dyn Emitter> = Arc::new(BroadcastEmitter(tx));
        let first = dir.clone();
        manager.rotate_file("test", &mut dir, &emitter).unwrap();
        let second = dir.clone();
        manager.rotate_file("test", &mut dir, &emitter).unwrap();
        assert_ne!(first, second);
        assert_ne!(second, dir);
        assert!(manager.is_recording("test"));
        assert!(manager.naturally_stopped.read().is_empty());
        assert_eq!(manager.sessions.read()["test"].dir_path, dir);
        assert!(
            crate::commands::recording_cmd::parse_timestamp_from_stem_pub(
                dir.file_name().unwrap().to_str().unwrap()
            )
            .is_some()
        );

        // Completed-file events must leave the streamer shown as recording.
        tokio::time::timeout(std::time::Duration::from_secs(5), async {
            let mut completed = 0;
            while completed < 2 {
                let event = events.recv().await.unwrap();
                if event.name == "recording-stopped" {
                    let payload: serde_json::Value = serde_json::from_str(&event.payload).unwrap();
                    assert_eq!(payload["is_recording"], true);
                    completed += 1;
                }
            }
        })
        .await
        .unwrap();
        manager.stop_recording("test").await.unwrap();
        assert_eq!(stop_rx.recv().await, Some(()));
